...
export const Tile: React.FC<TileProps> = ({ tile, onClick, isWon, charges, row }) => {
    ...
    // Performance optimization - memoize settings
    const perfSettings = useMemo(() => getPerformanceSettings(), []);

    useEffect(() => {
        // Skip spark effects on low-end devices
        if (hasFlow && !prevFlowRef.current && perfSettings.enableParticles) {
            const timer = setTimeout(() => {
                setShowSparks(true);
                setTimeout(() => setShowSparks(false), 800); // Faster cleanup
            }, flowDelay);
            return () => clearTimeout(timer);
        }
        prevFlowRef.current = hasFlow;
    }, [hasFlow, flowDelay, perfSettings.enableParticles]);
    ...
    const handleClick = () => {
        ...
        if (isLocked && fixed) {
            haptic.error(); // Locked tile haptic
            return;
        }
        if (fixed && !isTargetableBug) return;
        haptic.tileRotate(); // Normal tile click haptic
        onClick();
    };
